...
use Admin\Form\DamageForm;
use Admin\Model\Imagenlote;

class DetaildamageController extends AbstractActionController
{
    
    protected $damageTable;
        ...
        if($contenedor_excel == 0){
         ...
         if ($request->isPost()) {
             ...
             if ($form->isValid()) {
                ...
                $paginator = $this->getDamageTable()->Reporte($codigo_sap,$fecha,$turno,true);
                $paginator->setCurrentPageNumber((int) $this->params()->fromQuery('page', 1));
                $paginator->setItemCountPerPage(10);
                
                return new ViewModel(array(
                    'fecha' => $fecha,
                    'contenedor' => $codigo_sap,
                    'turno' => $turno,
                    'form' => $form,
                    'paginator' => $paginator,
                 ...
                 //return $this->redirect()->toRoute('admin-reporte-damage');
             
             }
         }else{
                $paginator = $this->getDamageTable()->Reporte(null,null,null,true);
                $paginator->setCurrentPageNumber((int) $this->params()->fromQuery('page', 1));
                $paginator->setItemCountPerPage(10);
                
                if($reporte == 'excel'){
                    $this->reporteAction($this->getDamageTable()->Reporte($contenedor_excel,$fecha_excel,$turno_excel,true));
    ...
    public function reporteAction($paginator)
    {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('J11', 'Caracteristicas daños  ')
            ->setCellValue('K11', 'Piezas dañadas  ')
            ->setCellValue('L11', 'Piezas paquete  ')
            ->setCellValue('M11', 'Condicion  ')
            ->setCellValue('N11', 'IMAGENES  ');
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('C7','DETALLE DE DAÑOS')
       ...
       $j = 1;
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('K'.$i, $m->piezas_danadas)
            ->setCellValue('L'.$i, $m->piezas_paquete)
            ->setCellValue('M'.$i, $m->estado); 
        ...
        endforeach;
          ...
          $objPHPExcel->getActiveSheet(0)->getStyle('B11:P'.$i)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER); 
          unset($styleArray);  
        // IMAGEN
    ...
    public function getDamageTable()
     {
         if (!$this->damageTable) {
             ...
             $this->damageTable = $sm->get('Admin\Model\DamageTable');
         }
